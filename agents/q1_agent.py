...
class Q1Agent(ValueEstimationAgent):
    ...
    def registerInitialState(self, gameState: GameState):
        ...
        if self.values is None:
            ...
            for _ in range(self.iterations):
                new_values = np.zeros((self.MDP.grid_width, self.MDP.grid_height))
                for state in self.MDP.getStates():
                    ...
                    new_values[state[0], state[1]] = best_q_value
    ...
    def computeQValueFromValues(self, state: tuple, action):
        ...
        for next_state, prob in self.MDP.getTransitionStatesAndProbs(state, action):
            ...
            q_value += prob * (reward + self.discount * self.values[next_state[0], next_state[1]])
        return q_value
